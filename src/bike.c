    ...
    else
    {
        ...
        if (collision > 0 && collision < COLLISION_VERTICAL_RAIL)
        {
            ...
            else
            {
                ...
                grindRunDirection = DIR_NONE;

                if (CheckSpeedchoiceOption(AUTO_MACH, AUTO_MACH_B) == TRUE && JOY_HELD(B_BUTTON) && gMapHeader.mapLayoutId != LAYOUT_FORTREE_CITY_GYM)
                    grindRunDirection = GetGrindRunDirection(direction);
                
                
                if(grindRunDirection != DIR_NONE)
                {
                    PlayerWalkFaster(grindRunDirection);
                    return;
                }
                else
                {
...
static u8 AcroBikeHandleInputWheelieStanding(u8 *newDirection, u16 newKeys, u16 heldKeys)
{
    ...
    if (gPlayerAvatar.bikeFrameCounter >= 20)
    {
        *newDirection = direction;
