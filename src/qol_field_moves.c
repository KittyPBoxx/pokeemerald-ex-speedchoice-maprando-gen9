...
u32 CanUseCut(s16 x, s16 y)
{
    ...
    if (GetObjectEventIdByPosition(x, y, 1) == OBJECT_EVENTS_COUNT && ((monHasMove && playerHasBadge)))
...
u32 CanUseSurf(s16 x, s16 y, u8 collision)
{
    ...
    bool32 playerHasBadge = FlagGet(FLAG_BADGE05_GET) == TRUE || (CheckSpeedchoiceOption(EARLYSURF, SURF_ON) && FlagGet(FLAG_BADGE03_GET));
...
u32 CanUseStrength(u8 collision)
{
    ...
    if (!playerUsedStrength && collisionEvent && ((monHasMove && playerHasBadge)))
        return FIELD_MOVE_POKEMON;
...
u32 UseWaterfall(struct PlayerAvatar playerAvatar, u32 fieldMoveStatus)
{
    HideMapNamePopUpWindow();
    LockPlayerAndLoadMon();
#ifdef QOL_NO_MESSAGING_WATERFALL
    FlagSet(FLAG_SYS_USE_WATERFALL);
#endif //QOL_NO_MESSAGING_WATERFALL
...
/**
* If a mon already knows the move use that mon
* Otherwise, if we have the hm just use any mon
*/
bool32 PartyHasMonLearnsKnowsFieldMove(u16 itemId)
{
    ...
    moveId = ItemId_GetSecondaryId(itemId);
    ...
    for (i = 0; i < PARTY_SIZE; i++)
    {
        ...
        if (MonKnowsMove(&gPlayerParty[i], moveId))
            return SetMonResultVariables(i,GetMonData(&gPlayerParty[i], MON_DATA_SPECIES));
    }
