...
const u8 gSpeedchoiceCurrentVersion[] = _("{COLOR GREEN}{SHADOW LIGHT_GREEN}v0.5.2 IX");
...
const struct OptionChoiceConfig OptionChoiceConfigSpeedup[MAX_CHOICES] = 
{
    { 120, (u8 *)&gSpeedchoiceTextOff },
    { 145, (u8 *)&gSpeedchoiceTextR   },
    { 180, (u8 *)&gSpeedchoiceTextOn  },
    { -1, NULL },
    { -1, NULL },
    { -1, NULL }
};
...
const struct SpeedchoiceOption SpeedchoiceOptions[CURRENT_OPTIONS_NUM + 1] = // plus one for page
{
    ...
    { 
        ...
        /* Option Tooltip */ gSpeedchoiceTooltipBattleSpeed,
        /* Option Usable  */ TRUE
    },
    // ----------------------------------
    // SPEEDUP OPTION
...
static void DrawPageChoice(u8 selection)
{
    ...
    AddTextPrinterParameterized(SPD_WIN_OPTIONS, SPEEDCHOICE_FONT_ID, text, 40, NEWMENUOPTIONCOORDS(5), TEXT_SKIP_DRAW, NULL);
}
...
static void Task_SpeedchoiceMenuProcessInput(u8 taskId)
{
	if(!gAlreadyLoaded)
	{
		DrawTooltip(taskId, gSpeedchoiceTooltipExplanation, TEXT_SKIP_DRAW, FALSE);
		gAlreadyLoaded = TRUE;
	}
...
static void DrawHeaderWindow(void)
{
    ...
    AddTextPrinterParameterized(SPD_WIN_TEXT_OPTION, SPEEDCHOICE_FONT_ID, gSpeedchoiceTextHeader, 4, 1, TEXT_SKIP_DRAW, NULL);
    ...
    AddTextPrinterParameterized(SPD_WIN_TEXT_OPTION, SPEEDCHOICE_FONT_ID, gSpeedchoiceCurrentVersion, 204 - width, 1, TEXT_SKIP_DRAW, NULL);
...
void DrawPageOptions(u8 page, u8 taskId) // Page is 1-indexed
{
    ...
    for(i = 0; i < drawCount; i++)
    {
        ...
        AddTextPrinterParameterized(SPD_WIN_OPTIONS, SPEEDCHOICE_FONT_ID, string, 4, NEWMENUOPTIONCOORDS(i), TEXT_SKIP_DRAW, NULL);
