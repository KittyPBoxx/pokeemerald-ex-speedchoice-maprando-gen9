...
static bool8 ExtractMonDataToSummaryStruct(struct Pokemon *mon)
{
    ...
    default:
        ...
        sMonSummaryScreen->relearnableMovesNum = FlagGet(P_SUMMARY_SCREEN_MOVE_RELEARNER) ? GetNumberOfRelearnableMoves(mon) : 0;
        return TRUE;
    }
    sMonSummaryScreen->switchCounter++;
    return FALSE;
...
static void Task_HandleInput(u8 taskId)
{
    if (MenuHelpers_ShouldWaitForLinkRecv() != TRUE && !gPaletteFade.active)
    {
        ...
        else if (JOY_NEW(START_BUTTON) && ShouldShowMoveRelearner()
...
static void Task_ChangeSummaryMon(u8 taskId)
{
    ...
    case 4:
        ...
        {
            if (FlagGet(P_SUMMARY_SCREEN_MOVE_RELEARNER) 
                && (sMonSummaryScreen->currPageIndex == PSS_PAGE_BATTLE_MOVES || sMonSummaryScreen->currPageIndex == PSS_PAGE_CONTEST_MOVES))
            {
...
{
    return (FlagGet(P_SUMMARY_SCREEN_MOVE_RELEARNER) 
