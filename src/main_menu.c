...
const u16 sMainMenuTextPal[] = INCBIN_U16("graphics/interface/main_menu_text.gbapal");
...
#define NUM_PRESET_NAMES min(ARRAY_COUNT(sMalePresetNames), ARRAY_COUNT(gFemalePresetNames))
...
static void Task_NewGameBirchSpeechSub_InitPokeBall(u8 taskId)
{
    ...
    CreatePokeballSpriteToReleaseMon(spriteId, gSprites[spriteId].oam.paletteNum, 112, 58, 0, 0, 32, PALETTES_BG, gUprStaticVars[BIRCH_INTRO_MON_INDEX]);
...
static void Task_NewGameBirchSpeechSub_WaitForLotad(u8 taskId)
{
    ...
    switch (tState)
    {
        case 0:
            ...
            break;
        case 1:
            if (gTasks[sBirchSpeechMainTaskId].tTimer >= 48)
            {
                DestroyTask(taskId);
                if (gTasks[sBirchSpeechMainTaskId].tTimer < 0x1000)
...
{
    int gender = (NewGameBirchSpeech_ProcessGenderMenuInput() ^ 1);
    int gender2;
...
{
    return CreateMonPicSprite_Affine(gUprStaticVars[BIRCH_INTRO_MON_INDEX], FALSE, 0, MON_PIC_AFFINE_FRONT, x, y, 14, TAG_NONE);
}
