...
static u8 GetEncounterLevelFromMapData(u16 species, u8 environment)
{
    ...
    hiddenTimeOfDay = (timeOfDay + 2) % TIMES_OF_DAY;
    ...
    case ENCOUNTER_TYPE_HIDDEN:
        ...
        {
            if (headbuttMonsInfo->wildPokemon[hiddenTimeOfDay][i].species == species)
            {
                min = (min < headbuttMonsInfo->wildPokemon[hiddenTimeOfDay][i].minLevel) ? min : headbuttMonsInfo->wildPokemon[hiddenTimeOfDay][i].minLevel;
                max = (max > headbuttMonsInfo->wildPokemon[hiddenTimeOfDay][i].maxLevel) ? max : headbuttMonsInfo->wildPokemon[hiddenTimeOfDay][i].maxLevel;
            }
        }
        
        // use encounter rate to signify is hidden pokemon are on land or in water
        if (headbuttMonsInfo->encounterRate == 1)
...
static bool8 CapturedAllHiddenMons(u16 headerId)
{
    ...
    if (headbuttMonsInfo != NULL)
    {
        for (i = 0; i < HIDDEN_WILD_COUNT; ++i)
        {
            species = headbuttMonsInfo->wildPokemon[hiddenTimeOfDay][i].species;
            if (species != SPECIES_NONE)
            {
                count++;
                if (!GetSetPokedexFlag(SpeciesToNationalPokedexNum(species), FLAG_GET_CAUGHT))
...
static void DexNavLoadEncounterData(void)
{
    ...
    // hidden mons
    if (landMonsInfo != NULL) // no encounter rate check since 0 means land, 1 means water encounters
    {
        for (i = 0; i < HIDDEN_WILD_COUNT; i++)
        {
            species = landMonsInfo->wildPokemon[hiddenTimeOfDay][i].species;
            if (species != SPECIES_NONE && !SpeciesInArray(species, 2))
                sDexNavUiDataPtr->hiddenSpecies[hiddenIndex++] = landMonsInfo->wildPokemon[hiddenTimeOfDay][i].species;
        }
    }
}
