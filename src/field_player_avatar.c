...
// static bool32 Fishing_RollForBite(bool32); Speedchoice: not used, fish always bit
...
static void PlayerNotOnBikeMoving(u8 direction, u16 heldKeys)
{
    ...
    if (gPlayerAvatar.flags & PLAYER_AVATAR_FLAG_SURFING)
    {
        ...
        if (FlagGet(FLAG_SYS_DEXNAV_SEARCH) && ((heldKeys & A_BUTTON) || (heldKeys & L_BUTTON)))
        {
    ...
    else if (FlagGet(FLAG_SYS_DEXNAV_SEARCH) && ((heldKeys & A_BUTTON) || (heldKeys & L_BUTTON)))
    {
        gPlayerAvatar.creeping = TRUE;
        PlayerGoSlow(direction);
    }
...
static void CreateStopSurfingTask(u8 direction)
{
    ...
    if (gPlayerAvatar.surfHistory == 1)
...
static void Task_WaitStopSurfing(u8 taskId)
{
    ...
    {
        if (gPlayerAvatar.surfHistory == 1)
            ObjectEventSetGraphicsId(playerObjEvent, GetPlayerAvatarGraphicsIdByStateId(PLAYER_AVATAR_STATE_MACH_BIKE));
        else 
...
static bool32 Fishing_GetRodOut(struct Task *task)
{
    ...
    const s16 minRounds2[] = {
        [OLD_ROD]   = 1,
        [GOOD_ROD]  = 1, // Speedchoice: less fishing
        [SUPER_ROD] = 1
    };
...
static bool32 Fishing_CheckForBite(struct Task *task)
{
    ...
    if(firstMonHasSuctionOrSticky)
        bite = TRUE; // bite = Fishing_RollForBite(firstMonHasSuctionOrSticky);

    if (!bite)
        bite = TRUE; // bite = Fishing_RollForBite(FALSE);
